...
Client.prototype.connect = function(name){
  ...
  var socket = nsp.add(this, function(){
    ...
    } else if (self.nspBuffers[name]) {
      var packets = self.nspBuffers[name];
      for (var i = 0; i < packets.length; i++) {
        socket.onpacket(packets[i]);
      }
      delete self.nspBuffers[name];
    }
  });
};
...
Client.prototype.ondecoded = function(packet) {
  if (parser.CONNECT == packet.type) {
    ...
    if (socket) {
      ...
      if (!this.nspBuffers[packet.nsp]) {
        this.nspBuffers[packet.nsp] = [];
      }
      this.nspBuffers[packet.nsp].push(packet);
    }
  }
};
